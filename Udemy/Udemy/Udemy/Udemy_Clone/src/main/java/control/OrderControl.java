		...
		try {
			...
			String deliveryAddress = request.getParameter("deliveryAddress");
			 ...
			 HttpSession session = request.getSession();
				...
				email.setFrom("doanjava123@gmail.com"); //chinh lai email quan tri tai day [chu y dung email con hoat dong]
				email.setFromPassword("fdqd ywuy dkie fziz"); //mat khau email tren
				email.setTo(emailAddress);
				email.setSubject("Dat hang thanh cong tu UDEMY");
				StringBuilder sb = new StringBuilder();
				sb.append("Dear ").append(name).append("<br>");
				sb.append("Ban vua dat dang tu UDEMY. <br> ");
				sb.append("QUOC GIA BAN SINH SONG LA <b>").append(deliveryAddress).append(" </b> <br>");
				sb.append("So dien thoai  cua ban la: <b>").append(phoneNumber).append(" </b> <br>");
				sb.append("Cac san pham ban dat la: <br>");
				for(Cart c : list) {
					for(Product p : list2) {
						if(c.getProductID()==p.getId()) {
							sb.append(p.getName()).append(" | ").append("Price:").append(p.getPrice()).append("$").append(" | ").append("Amount:").append(c.getAmount()).append(" | ").append("Duration:").append(c.getSize()).append("<br>");
						}
					}
				}
				sb.append("Tong Tien: ").append(String.format("%.02f",totalMoneyVAT)).append("$").append("<br>");
				sb.append("Cam on ban da dat hang tai UDEMY<br>");
				sb.append("Thân chào");
